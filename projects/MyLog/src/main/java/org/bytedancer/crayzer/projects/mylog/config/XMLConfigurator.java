...
import org.bytedancer.crayzer.projects.mylog.appender.AppenderBase;
...
import org.bytedancer.crayzer.projects.mylog.util.StringUtils;
...
public class XMLConfigurator implements IConfigurator {
    ...
    private Map<String, IAppender> appenderCache = new HashMap<>();
    ...
    private void parseLoggers(Element document) throws IllegalAccessException, ClassNotFoundException, InstantiationException {
        ...
        for (int i = 0; i < loggerNodeList.getLength(); i++) {
            DefaultLogger logger = parseChildrenOfLoggerElement((Element) loggerNodeList.item(i));
    ...
    private void parseRoot(Element document) throws IllegalAccessException, ClassNotFoundException, InstantiationException {
        ...
        DefaultLogger root = parseChildrenOfLoggerElement(rootEle);
        loggerContext.setRoot(root);
    }

    private DefaultLogger parseChildrenOfLoggerElement(Element element) throws InstantiationException, IllegalAccessException, ClassNotFoundException {
        ...
        for (int i = 0; i < refLength; i++) {
            ...
            IAppender appender = findAppenderByName(element.getOwnerDocument(), appenderName);
            aai.addAppender(appender);
        }
        return logger;
    }

    protected IAppender findAppenderByName(Document document, String appenderName) throws ClassNotFoundException, InstantiationException, IllegalAccessException {
        IAppender appender = appenderCache.get(appenderName);
        if (appender != null) {
            return appender;
        }
        ...
        for (int i = 0; i < appenderNodeList.getLength(); i++) {
            ...
            if (appenderName.equals(itemAppenderName)) {
                ...
                appender = (IAppender) Class.forName(itemAppenderClassName).newInstance();
    ...
    private void startComponent(ILifeCycle component) {
        component.start();
    }
